...
Sous les hypothèses du modèle, les estimateurs MCO $(\hat{\beta}_0, \hat{\beta}_1)$ sont les estimateurs \textbf{linéaires} et \textbf{non-biaisés} de $\beta_0, \beta_1$ ayant la variance minimale.  
...
%\begin{figure}[H]
%\centering
%\includegraphics[width=8cm, height=6cm]{Regression_Estimation.jpg}
...
\commentaire{ D’après les formules connues :
\[
...
\commentaire{\textbf{Attention :} le dénominateur est bien $n-2$ (et non $n$), car deux paramètres
...
\commentaire{ On a donc que $\hat{y}$ est la projection de $y$ sur $\mathcal{M}(x)$ 

%Image{\label{image_projection}} 
}
