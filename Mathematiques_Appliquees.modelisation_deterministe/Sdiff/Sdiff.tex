...
\item Soit $\Phi$ et $\Gamma$ deux solutions de (\ref{equa:5.1}) tells que $\Gamma(0)$ soit \textit{proche} de $\Phi(0)$,c'est-à-dire pour $j \in \{ 1, \cdots , n \} , \Gamma_j (0)$ \textit{proche} de $ \Phi_j(0) $. La solution $\Gamma$ restera t'elle \textit{proche} de $\Phi(t)$ ou s'en \textit{éloignera} t-elle lorsque $t$ tend vers l'infini?
...
Nous voulons savoir quand $x^*$ est stable, asymptotiquement stable ou instable, c'est-à-dire nous voulons savoir si tout solution $\Phi$ de (\ref{equa:5.2}) \textit{suffisamment proche} de $x^*$ à l'instant $t=0$, reste \textit{proche} de $x^*$ pour tour instant $t>0$.

Cette notion est due au mathématicien russe Aleksandr Lyapounov (1857-1918).

\begin{defi}[Stabilité, Stabilité asymptotique, Instabilité]
\begin{enumerate}
\item U point d'équilibre $x^*$ de (\ref{equa:5.2}) est stable si toute solution $\Phi$ de (\ref{equa:5.2}) \textit{suffisamment proche} de $x^*$ à l'instant $t=0$ reste \textit{proche} de $x^*$ pour tout $t>0$. Autrement dit, $x^*$ est stable si pour tout $\varepsilon >0$, il existe un $\delta >0$, tel que pour toute solution $\Phi$ de (\ref{equa:5.2}), on ait:
...
\item $x^*$ est dit instable si $x^*$ n'est pas stable. Autrement dit, s'il existe une solution $\Phi$ de (\ref{equa:5.2}) qui soit \textit{proche} de $x^*$ à $t=0$ mais qui ne le soit plus ("proche") pour $t>0$.
\end{enumerate}
\end{defi}
...
\begin{defi}[Hyperbolicité d'un point d'équilibre de (\ref{equa:5.3})]
...
\begin{defi}[Hyperbolicité d'un point d'équilibre de \ref{equa:5.6}]
